...
import { auth, db, acceptFriendRequestCallable } from '../../firebase';
...
const FriendRequests = ({ onBack, onStartChat }) => {
    ...
    const handleAcceptRequest = async (requestId, senderId) => {
        ...
        try {
            const result = await acceptFriendRequestCallable({ requestId });

            if (result.data.success) {
                setActionStatus(prev => ({ ...prev, [requestId]: 'success' }));
                console.log('Friend request accepted:', requestId);
            } else {
                throw new Error(result.data.message || 'Failed to accept request.');
            }
