...
import { auth, db, broadcastMessageCallable, deleteUserCallable, updateAnalyticsCallable, updateUserAdminCallable, updateConfigAdminCallable } from '../../firebase';
...
const DashboardView = () => {
  ...
  useEffect(() => {
    // Initial fetch via Cloud Function to bypass potential rule issues on load
    updateAnalyticsCallable().then(result => {
      if (result.data && result.data.data) {
        setStats(result.data.data);
      }
    }).catch(err => console.error("Initial analytics fetch failed:", err));

    // Real-time listener (might fail if rules are strict, but we have the initial fetch)
    const unsub = onSnapshot(doc(db, 'analytics', 'summary'), (doc) => {
      if (doc.exists()) { setStats(doc.data()); }
    }, (error) => {
      console.log("Snapshot listener permission error (using manual refresh instead):", error);
    });
    return () => unsub();
  }, []);
  ...
  const handleRefreshStats = async () => {
    ...
    try {
      const result = await updateAnalyticsCallable();
      if (result.data && result.data.data) {
        setStats(result.data.data);
      }
    } catch (error) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={handleRefreshStats} disabled={isRefreshing} className="refresh-stats-btn">
...
const UsersView = () => {
  ...
  // --- Updated Action Handlers using Cloud Functions ---

  const handleToggleBan = async (userId, currentStatus) => {
    try {
      await updateUserAdminCallable({ targetUserId: userId, updates: { isBanned: !currentStatus } });
      setUsers(users.map(u => u.id === userId ? { ...u, isBanned: !currentStatus } : u));
    } catch (error) {
      console.error("Error toggling ban:", error);
      alert("Failed to toggle ban: " + error.message);
    }
  };

  const handlePlanChange = async (userId, newPlan) => {
    try {
      await updateUserAdminCallable({ targetUserId: userId, updates: { plan: newPlan } });
  ...
  const handleResetCount = async (userId) => {
    try {
      await updateUserAdminCallable({ targetUserId: userId, updates: { dailyMessageCount: 0 } });
              ...
              <tr key={user.id}>
                ...
                <td>
                  <div className="action-buttons">
                    ...
                    <button onClick={() => handleDeleteUser(user.id)} className="action-btn-ban" style={{ backgroundColor: '#ef4444', marginLeft: '5px' }}>
                      Delete
                    </button>
                  </div>
                </td>
...
const ConfigView = () => {
  ...
  const handleSave = async (e) => {
    ...
    setMessage('');
    try {
      // Use Cloud Function to save settings
      await updateConfigAdminCallable({ settings });
...
const BroadcastView = () => {
  ...
  const handleSendBroadcast = async (e) => {
    ...
    try {
      const result = await broadcastMessageCallable({
        message: message,
        target: target
      });
      ...
      <form onSubmit={handleSendBroadcast} className="broadcast-form">
        <div className="form-field">
          ...
          <select value={target} onChange={(e) => setTarget(e.target.value)}>
            ...
            <option value="web">Webium Users</option>
            <option value="mobile">Mobilum Users</option>
            <option value="omnium">Omnium Users</option>
          </select>
        </div>
        <div className="form-field">
          <label>Message</label>
