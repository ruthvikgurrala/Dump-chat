...
const ChatWindow = ({ selectedChat, onBack, onMessageSent }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(qLive, (snapshot) => {
      ...
      // Pagination Logic:
      // If snapshot is empty, we have no cursor.
      // We update the cursor if we have MORE data than we've ever seen before for this chat load.
      // This handles the case where initial cache has < 20 items (hasMore=false), 
      // but then server gives 20 items (hasMore=true).
      if (snapshot.empty) {
        setLastDoc(null);
        setHasMore(false);
  ...
  useLayoutEffect(() => {
    ...
    // (The "own message" part is implicitly handled by isAtBottomRef usually being true when typing, 
    // but we can force it for optimistic updates if needed. For now, relying on isAtBottomRef is safer UX)
    if (isAtBottomRef.current) {
      el.scrollTop = el.scrollHeight;
    }
  }, [messages, loadingMore]); // Depend on messages array
  ...
  const handleSendMessage = async (e) => {
    ...
    setNewMessage('');
    // const timestamp = new Date(); // Use client time for optimistic update - no longer directly used
    ...
    const messageDataForFirestore = {
      ...
      seen: false,
      edited: false,
      clientMessageId: clientMessageId, // Add unique client ID
    };

    // Optimistic update
    const optimisticMessage = {
      id: `temp-${clientMessageId}`, // Use client ID for temp ID
    ...
    isAtBottomRef.current = true; // Force scroll to bottom for own message
