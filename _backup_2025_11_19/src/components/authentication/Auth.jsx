...
const Auth = ({ authError, setAuthError }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        email: email,
        autoDump: true,
        dailyMessageCount: 0, // <-- ADD THIS LINE
        plan: 'free', // Also a good idea to explicitly set the plan
      });
  ...
  // UPDATED: handleLogin is now much simpler and no longer checks for bans
  const handleLogin = async () => {
    setAuthError('');
    setMessage('');
    try {
      // This function's only job is to sign the user in.
      // The ban check is now handled centrally in App.jsx.
